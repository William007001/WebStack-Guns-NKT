...
Site.initColumn = function () {
    var columns = [
        ...
        {
            title: '地址', field: 'sitUrl', align: 'center', valign: 'middle', sortable: true,
            formatter: function (value, obj) {
                if (obj.uatUrl || obj.prodUrl) {
                    var url = [];
                    url.push('<span>SIT地址：' + (obj.sitUrl ? obj.sitUrl : '无') + '</span><br>');
                    url.push('<span>UAT地址：' + (obj.uatUrl ? obj.uatUrl : '无') + '</span><br>');
                    url.push('<span>PROD地址：' + (obj.prodUrl ? obj.prodUrl : '无') + '</span>');
                    return url.join('');
                } else {
                    return value;
                }
